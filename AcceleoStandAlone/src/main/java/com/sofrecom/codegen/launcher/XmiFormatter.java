...
public class XmiFormatter {
	...
	public XmiFormatter(final String entityname , final String packageName, final String classesPath ){
		this.baseDtoPath = packageName;
		this.entityName = entityname;
		fieldsFromDto = this.getFieldsFromDto(
				classesPath, String.format("%s.%s", baseDtoPath, entityName));
	 ...
	 * 
	 * @param classesPath
	 *            : root path of the class directory
	 * @param fullQualifiedClassName
	 *            : the name of the class proceeded with the the package name
	...
	private ViewModel buildViewModel() {
		...
		viewModel.getEntities().add(this.buildEntity());
		return viewModel;
	}
	...
	private Entity buildEntity() {
		...
		for (String key : fieldsFromDto.keySet()) {
			Attribute attribute = CodegenFactory.eINSTANCE.createAttribute();
			attribute.setName(key);
			attribute.setType(fieldsFromDto.get(key));
	...
	/**
	 * register an xmi resource factory to save a file with xmi extension outside Eclipse without having null pointer exception
	 */
	...
	private File createResourceFileIfNotExists() {
		// create a resource
		final File file = new File(String.format("./model/%s.xmi", entityName));
		if (!file.exists()) {
			try {
				file.createNewFile();
	...
	/**
	 * build and save the xmi file from a model charged from the parsing result of a POJO (DTO almost in all cases)
	 * for the parsing process  we use the classes directory and the name of the entity.   
	 */
